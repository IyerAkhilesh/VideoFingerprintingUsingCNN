...
class CNN:
    ...
    def __init__(self, num_features, num_classes):
        ...
        # layer 2
        model.add(Conv1D(num_filters[2], filter_sizes[0], activation='relu'
                    , kernel_regularizer=l2(l2_lambda)))
        model.add(Conv1D(num_filters[3], filter_sizes[1], activation='tanh',
                         strides = 2, kernel_regularizer=l2(l2_lambda)))
        model.add(Conv1D(num_filters[0], filter_sizes[0]
        ...
        model.add(Dropout(0.33))  # Decreasing the dropout rate to an appropriate value provides finer tuning and more accuracy
...
def main():
    ...
    X_train, Y_train, X_test, Y_test = split_data(X, Y, 0.6, balance_dist=False)
    ...
    # with open("D:\\DatasetAndCSV\\OutputAndScreenshots\\model_rtt_85-15-lr-0005.json", "w") as json_file:
    #     json_file.write(json_model)
    #
    # # Serialize weights to h5 file
    # model.model.save_weights("D:\\DatasetAndCSV\\OutputAndScreenshots\\model_rtt_85-15-lr-0005.h5")
